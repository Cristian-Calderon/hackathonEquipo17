...
async function searchProductsJSON()
{
    const requestURL = "../data.json";
    const response = await fetch(requestURL);
    const productsSearch = await response.json();
    
    const shop = await document.getElementById("products");
    shop.innerHTML = "";
    for (let index = 0; index < productsSearch.listOfProducts.length; index++) 
    {
        let name = productsSearch.listOfProducts[index].nombre;
        let img = productsSearch.listOfProducts[index].imagen;
        let price = productsSearch.listOfProducts[index].precio;
        let contact = productsSearch.listOfProducts[index].link;
        let id = productsSearch.listOfProducts[index].id;
        let category = productsSearch.listOfProducts[index].categoria;
        let description = productsSearch.listOfProducts[index].descripcion;

        const search = document.getElementById("searchBox").value

        var nameCaps = name.toUpperCase();
        var searchCaps = search.toUpperCase();
        var categoryCaps = category.toUpperCase();
        var descriptionCaps = description.toUpperCase();
        if (nameCaps.includes(searchCaps) || categoryCaps.includes(searchCaps) || descriptionCaps.includes(searchCaps)) 
        {
            shop.innerHTML += `
